...
export function createCQRSUserRoutes(controller: CQRSUserController): Router {
  ...
  // Requête agrégée - doit être AVANT /users/:id pour éviter le conflit de routes
  router.get('/users/:id/with-accounts', (req, res) => controller.getUserWithAccounts(req, res));
