...
export class CQRSUserController {
  ...
  // Requête agrégée - retourne utilisateur avec ses comptes
  async getUserWithAccounts(req: Request, res: Response): Promise<void> {
    try {
      ...
      const request = new GetUserWithAccountsRequest(id);
      const userWithAccounts = await this.mediator.send(request);
      ...
      res.status(200).json({ success: true, data: userWithAccounts });
    } catch (error: any) {
      this.handleError(error, res, 500);
    }
  }
